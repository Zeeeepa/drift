/**
 * Semantic HTML Detector - SEMANTIC VERSION
 * 
 * Language-agnostic detector that finds semantic HTML patterns
...
export class SemanticHtmlSemanticDetector extends SemanticDetector {
  ...
  protected createPatternViolation(
    ...
    return {
      ...
      message: `Inconsistent semantic HTML pattern: using '${match.contextType}' but project primarily uses '${dominantPattern.contextType}'`,
      expected: dominantPattern.contextType,
      actual: match.contextType,
      explanation: `Your project uses '${dominantPattern.contextType}' for semantic HTML in ${dominantPattern.percentage.toFixed(0)}% of cases ` +
...
export function createSemanticHtmlSemanticDetector(): SemanticHtmlSemanticDetector {
  return new SemanticHtmlSemanticDetector();
}
