/**
 * Near Duplicate Detector - SEMANTIC VERSION
 * 
 * Language-agnostic detector that finds near-duplicate component patterns
...
export class NearDuplicateSemanticDetector extends SemanticDetector {
  readonly id = 'components/near-duplicate';
  readonly name = 'Near Duplicate Detector';
  readonly description = 'Learns near-duplicate component patterns from your codebase';
  ...
  protected createPatternViolation(
    ...
    return {
      ...
      explanation: `Your project uses '${dominantPattern.contextType}' for similar components in ${dominantPattern.percentage.toFixed(0)}% of cases ` +
...
export function createNearDuplicateSemanticDetector(): NearDuplicateSemanticDetector {
  return new NearDuplicateSemanticDetector();
}
